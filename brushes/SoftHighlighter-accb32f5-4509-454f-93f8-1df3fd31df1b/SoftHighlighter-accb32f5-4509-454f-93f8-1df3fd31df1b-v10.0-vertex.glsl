...
void main() {
  ...
  vec3 audioPos = localPos + a_normal * (u_BeatFFT.z * 0.15 * sin(a_texcoord0.x * 20.0 - u_time.y * 10.0));
  localPos = mix(localPos, audioPos, audioActive);
  ...
  v_position = (modelViewMatrix * vec4(localPos, 1.0)).xyz;
  ...
  audioColor.rgb = a_color.rgb * 0.5 + a_color.rgb * u_BeatFFT.z;
