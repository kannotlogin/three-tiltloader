...
void main() {
    ...
    float accumBeat = u_time.y + highMidBeat * 3.0;
    texcoord.y += texcoord.x * 3.0 + mix(0.0, accumBeat, audioActive);

    float timeOffset = u_time.w + mix(0.0, bassBeat * 10.0, audioActive);

    float waveform_r = .15 * sin( -20.0 * unbloomedColor.r * timeOffset + texcoord.x * 100.0 * unbloomedColor.r);
    float waveform_g = .15 * sin( -30.0 * unbloomedColor.g * timeOffset + texcoord.x * 100.0 * unbloomedColor.g);
    float waveform_b = .15 * sin( -40.0 * unbloomedColor.b * timeOffset + texcoord.x * 100.0 * unbloomedColor.b);
